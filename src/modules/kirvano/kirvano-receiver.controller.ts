...
// instead the security-token header, checked in KirvanoReceiverService against the per-session token
...
export class KirvanoReceiverController {
  ...
  @ApiResponse({ status: 401, description: 'Missing or invalid security-token header.' })
  ...
  async receive(
    ...
    this.logger.log('Kirvano webhook received', { sessionId, headers });
