...
// Scoped to the DATA-owned modules only (session/webhook/message/template/engine/integration/
// status-store/automation/kirvano), mirroring the runtime data connection (app.module.ts). A broad
// '**' glob would also sweep in the main-owned auth/audit entities and pollute `migration:generate`
...
const dataEntities = [
  ...
  sourceGlob('..', 'modules', 'kirvano', '**', '*.entity{.ts,.js}'),
];
